...
def add_orgs_to_graph(graph):
    ...
    graph.add((univ_elem, RDFS.label, Literal(UNIVERSITY["name"], lang='en')))
    graph.add((univ_elem, VIVO.overview, Literal(UNIVERSITY["desc"], lang='en')))

    division_elem = NS[ACADEMIC_AFFAIRS["uid"]]
    graph.add((division_elem, RDF.type, VIVO.Division))
    graph.add((division_elem, RDFS.label, Literal(ACADEMIC_AFFAIRS["name"], lang='en')))
    graph.add((division_elem, VIVO.overview, Literal(ACADEMIC_AFFAIRS["desc"], lang='en')))
    ...
    for dept_name, dept_uid in ACADEMIC_AFFAIRS["depts"].items():
        ...
        graph.add((dept_elem, RDFS.label, Literal(dept_name, lang='en')))
    ...
    for coll, coll_details in COLL_DEPT.items():
        ...
        graph.add((coll_elem, RDFS.label, Literal(coll, lang='en')))
